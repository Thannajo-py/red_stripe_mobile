...
import thannajo.appli.filrouge.databinding.ActivityAddElementBinding
...
abstract class AddElementActivity : CommonTypeAbstractActivity(), View.OnClickListener,
    ...
    private fun<T, U> setImage(name: String, dao: CommonDao<T, U>){
        CoroutineScope(SupervisorJob()).launch{
            ...
            if(imageList.isNotEmpty()){
                val file = File(this@AddElementActivity.filesDir, imageList.first().name)
    ...
    private fun setImage(imageName:String){
        val file = File(this@AddElementActivity.filesDir, imageName)
        val compressedBitMap = BitmapFactory.decodeByteArray(
            file.readBytes(),
            0,
            file.readBytes().size
    ...
    private fun registerObject(name:String){
        ...
        startActivity(Intent(this@AddElementActivity, ViewGamesActivity::class.java))
        finish()
        endLoading()
    }
    ...
    private fun setAddOnRv(adapter: GenericCommonGameListCbAdapter<CommonGame>){
        runOnUiThread {
            ...
            appInstance.database.addOnDao()
                ...
                .observe(this@AddElementActivity, { it?.let { adapter.submitList(it) } })
        }
    }
