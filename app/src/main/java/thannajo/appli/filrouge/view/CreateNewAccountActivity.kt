...
import thannajo.appli.filrouge.databinding.ActivityCreateNewAccountBinding
import thannajo.appli.filrouge.model.UserTableBean
import thannajo.appli.filrouge.utils.RegexPattern
import thannajo.appli.filrouge.utils.currentUser
import thannajo.appli.filrouge.utils.generateHashedPass
...
class CreateNewAccountActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private fun createUser(login:String, password:String){
        ...
        try{
           val encryptedPassword = generateHashedPass(password)!!
            ...
            runOnUiThread {
                Toast.makeText(
                    this@CreateNewAccountActivity,
                    getString(R.string.success),
                    Toast.LENGTH_LONG
                ).show()
                startActivity(Intent(this@CreateNewAccountActivity, ViewGamesActivity::class.java))
                finish()
            }
        }
        catch(e:Exception){
