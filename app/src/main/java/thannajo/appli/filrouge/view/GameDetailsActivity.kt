...
import thannajo.appli.filrouge.databinding.ActivityGameDetailsBinding
...
class GameDetailsActivity : DetailsCommonMethodAbstractActivity(){
    ...
    private fun fillGameSpecificRv(){
        ...
        dbMethod.getGameCommonSpecificField().forEach{
            CoroutineScope(SupervisorJob()).launch{
                ...
                val adapter = GenericStringListAdapter(this@GameDetailsActivity, it)
                val field = appInstance.database.gameDao().getMember("get${it}s", gameId)
                        as LiveData<List<ID>>
                runOnUiThread {
                    rv.adapter = adapter
                    layout(rv)
                    field.observe(this@GameDetailsActivity, {it?.let{adapter.submitList(it)}})
                }
            }
        }
    }
