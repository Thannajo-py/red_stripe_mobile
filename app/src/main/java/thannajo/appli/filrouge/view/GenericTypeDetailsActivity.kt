...
import thannajo.appli.filrouge.databinding.ActivityGenericTypeDetailsBinding
import thannajo.appli.filrouge.model.CommonGame
import thannajo.appli.filrouge.model.DesignerWithGame
...
class GenericTypeDetailsActivity : CommonTypeAbstractActivity() {
    ...
    private fun handleSearchQuery(){
        ...
        CoroutineScope(SupervisorJob()).launch{
            if (search.topic != null || search.mechanism != null || search.tag != null){
                val adapter = GenericListAdapter(this@GenericTypeDetailsActivity)
                handleGameSearchQuery(search, adapter)
                displayRv(
                    binding.pbRvGenericDetailAddOn,
                    binding.rvGenericDetailAddOn
                )
                displayRv(
                    binding.pbRvGenericDetailMultiAddOn,
                    binding.rvGenericDetailMultiAddOn
                )
            }
            else {
    ...
    private fun handleOtherSearchQuery(it: String, search:SearchQuery, adapter: GenericListAdapter){
        ...
        runOnUiThread {
            ...
            layout(rv)
            data.observe(
                this@GenericTypeDetailsActivity,
